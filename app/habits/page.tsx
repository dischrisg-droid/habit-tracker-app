...
export default function HabitsPage() {
  ...
  const [form, setForm] = useState({ name: '', frequency: 'daily' as const, days: [] as number[], targettime: '', notes: '' });
  ...
  const save = async () => {
    ...
    const h = editing ? { ...editing, ...form, name: form.name.trim() } : { id: crypto.randomUUID(), ...form, name: form.name.trim() };
    await saveHabits(editing ? habits.map(x => x.id === editing.id ? h : x) : [...habits, h]);
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b">
        <div className="max-w-5xl mx-auto px-6 py-6 flex items-center justify-between">
          ...
          <button
            ...
            className="px-8 py-5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-2xl font-bold rounded-2xl shadow-2xl hover:scale-110 transition flex items-center gap-3"
          >
            <Plus className="w-8 h-8" /> New
          </button>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto p-8">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {habits.map(habit => {
            ...
            const done = todayLog?.completedHabits.includes(habit.id);

            return (
              <div key={habit.id} className="group relative bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden hover:-translate-y-4 hover:shadow-3xl transition-all duration-500">
                {s > 0 && (
                  <div className="absolute -top-6 -right-6 bg-gradient-to-br from-orange-500 to-pink-600 text-white px-7 py-4 rounded-full font-black text-3xl shadow-2xl flex items-center gap-2 animate-pulse">
                    <Flame className="w-10 h-10" />{s}
                    {s >= 21 && <Sparkles className="w-9 h-9" />}
                  </div>
                )}
                ...
                <div className="p-10">
                  ...
                  <div className="space-y-4 text-lg">
                    <p className="font-medium text-indigo-600">
                      {habit.frequency === 'daily' ? 'Daily' : 'Weekly'} {habit.targettime && `· ${habit.targettime}`}
                    </p>
                    {habit.notes && <p className="italic text-gray-600 bg-gray-50 px-5 py-3 rounded-2xl">“{habit.notes}”</p>}
                  </div>

                  <div className="mt-10 flex justify-between items-center">
                    <span className={`text-2xl font-bold ${done ? 'text-green-600' : 'text-gray-400'}`}>
                      {done ? 'Done' : 'Pending'}
                    </span>
        ...
        {habits.length === 0 && (
          <div className="text-center py-40">
            <h2 className="text-7xl font-black text-gray-700 mb-8">No habits yet</h2>
            <button onClick={() => setShowForm(true)} className="px-20 py-10 text-4xl font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600 rounded-3xl shadow-2xl hover:scale-110 transition">
              Start Building
            </button>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl flex items-center justify-center p-6 z-50">
          <div className="bg-white rounded-3xl shadow-3xl p-12 max-w-2xl w-full">
            <h2 className="text-5xl font-black text-center mb-12 bg-gradient-to-r from-indigo-600 to-pink-600 bg-clip-text text-transparent">
              {editing ? 'Edit' : 'New'} Habit
            </h2>
            <input autoFocus placeholder="Habit name" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
            ...
            <div className="grid grid-cols-2 gap-8 mb-10">
              {(['daily','weekly'] as const).map(f => (
                <label key={f} className="flex items-center gap-4 text-xl">
                  <input type="radio" checked={form.frequency===f} onChange={() => setForm({...form, frequency: f, days: f==='daily'?[]:form.days})}
                    className="w-6 h-6 text-indigo-600" />
                  <span className="font-bold">{f==='daily'?'Daily':'Choose days'}</span>
                </label>
              ))}
            </div>
              ...
              <div className="flex justify-center gap-8 mb-10">
                {['S','M','T','W','T','F','S'].map((d,i) => (
                  <label key={i} className="text-xl font-bold">
                    <input type="checkbox" checked={form.days.includes(i)} onChange={e => setForm({...form, days: e.target.checked ? [...form.days,i] : form.days.filter(x=>x!==i)})}
                      className="w-6 h-6 mb-2" /><div>{d}</div>
                  </label>
                ))}
              </div>
            )}
            <input placeholder="Best time (optional)" value={form.targettime} onChange={e => setForm({...form, targettime: e.target.value})}
              className="w-full px-8 py-6 text-xl rounded-2xl border-4 border-pink-200 focus:border-pink-500 outline-none mb-8" />
            <textarea placeholder="Why this matters" rows={4} value={form.notes} onChange={e => setForm({...form, notes: e.target.value})}
            ...
            <div className="flex gap-6 mt-12">
              ...
              <button onClick={() => setShowForm(false)} className="px-12 py-7 text-3xl font-bold bg-gray-200 rounded-2xl">Cancel</button>
            </div>
          </div>
        </div>
      )}
